  ...
  const textContent = below800 ? (
    <div>
      <Text fontWeight={500} lineHeight={'145.23%'} mt={'10px'}>
        Anyone can create and name any HRC20 token on Heco Chain, including creating fake versions of existing tokens and
        tokens that claim to represent projects that do not have a token.
      </Text>
      <Text fontWeight={500} lineHeight={'145.23%'} mt={'10px'}>
        Similar to Etherscan, this site automatically tracks analytics for all HRC20 tokens independent of token
          ...
          <div>
            <Hover style={{ marginTop: '10px' }}>
              <Link
                ...
                target="_blank"
              >
                View {type === 'token' ? 'token' : 'pair'} contract on Heco
              </Link>
            </Hover>
